...
router.get('/', auth, async (req, res) => {
  ...
  if (stores && stores.length === 0) return res.status(404).send({ msg: 'No stores found.' });
...
router.get('/:id', [validateObjectId, auth], async (req, res) => {
  ...
  if (!store) return res.status(400).send({ msg: 'Store with the given ID not found.' });
...
router.get('/admin/:id', [validateObjectId, auth, admin], async (req, res) => {
  ...
  if (stores && stores.length === 0) return res.status(400).send({ msg: 'Stores with the given admin ID not found.' });
...
router.get('/manager/:id', [validateObjectId, auth], async (req, res) => {
  ...
  if (stores && stores.length === 0) return res.status(400).send({ msg: 'Stores with the given manager ID not found.' });
...
router.get('/team/:id', [validateObjectId, auth], async (req, res) => {
  ...
  if (stores && stores.length === 0) return res.status(400).send({ msg: 'Stores with the given team ID not found.' });
});
...
router.get('/me', auth, async (req, res) => {
  ...
  if (teams && teams.length === 0) return res.status(400).send({ msg: 'Member not registered to any team.' });

  const stores = teams.map(async team => {
    return await Store.find({ team_id: team._id });
  });

  if (stores && stores.length === 0) return res.status(404).send({ msg: 'Team does not have any stores.' });
...
router.post('/', [auth, admin], async (req, res) => {
  ...
  if (swearjar.profane(store_name)) return res.status(400).send({ msg: 'Store name must not contain profanity.' });
  if (swearjar.profane(store_message)) return res.status(400).send({ msg: 'Store message must not contain profanity.' });

  if (!validateId(admin_id)) return res.status(400).send({ msg: 'Invalid ID. (Admin)' });
  const admin = await Member.findById(admin_id);
  if (!admin) return res.status(400).send({ msg: 'Admin with the given ID not found.' });

  if (!validateId(manager_id)) return res.status(400).send({ msg: 'Invalid ID. (Manager)' });
  const manager = await Member.findById(manager_id);
  if (!manager) return res.status(400).send({ msg: 'Manager with the given ID not found.' });
...
router.patch('/add/item/:id', [validateObjectId, auth, admin], async (req, res) => {
  ...
  if (!validateId(item_id)) return res.status(400).send({ msg: 'Invalid ID. (Item)' });
  const catalogItem = await CatalogItem.findById(item_id);
  if (!catalogItem) res.status(400).send({ msg: 'Catalog Item with the given ID not found.' });
...
router.delete('/:id/item/:itemId', [validateObjectId, auth, admin], async (req, res) => {
  let store = await Store.findById(req.params.id);
  if (!store) res.status(400).send({ msg: 'Store with the given ID not found.' });

  if (!validateId(req.params.itemId)) return res.status(400).send({ msg: 'Invalid ID. (Item)' });
